...
% if page > 0: # always give a link to the first page of results
  <a class="pagenum" href="{{pagelink}}0" class="btn btn-altcolor">1</a>
% end
...
% elif page == 4: # if the ellipsis would only eliminate one number, just print the page number:
  <a class="pagenum" href="{{pagelink}}1" class="btn btn-altcolor">2</a>
% end

% if page > 2:
  <a class="pagenum" href="{{pagelink}}{{ page - 2 }}" class="btn btn-altcolor">{{ page - 1 }}</a>
% end
% if page > 1:
  <a class="pagenum" href="{{pagelink}}{{ page - 1 }}" class="btn btn-altcolor">{{ page }}</a>
% end
...
% for i in range(1, maxpages):
  % if (page + i) * page_size < totalcount:
    <a class="pagenum" href="{{pagelink}}{{ page + i }}" class="btn btn-altcolor">{{ page + i + 1 }}</a>
    % page_nums_printed += 1
  % end
% end
...
% if last_printed < lastpage: # if the final page printed isn't the last page
  ...
  % elif last_printed  == lastpage - 1:
    <a class="pagenum" href="{{pagelink}}{{ lastpage - 1 }}" class="btn btn-altcolor">{{ lastpage }}</a>
  % end
  <a class="pagenum" href="{{pagelink}}{{ lastpage }}" class="btn btn-altcolor">{{ lastpage  + 1 }}</a>
% end
